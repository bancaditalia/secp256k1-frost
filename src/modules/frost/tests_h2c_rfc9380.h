...
void test_h2c_sqrt_ratio(void) {
    unsigned long i;
    unsigned long ietf_h2c_sqrt_ratio_num_test_vectors;

    ietf_h2c_sqrt_ratio_num_test_vectors = sizeof(sqrt_ratio_vectors) / sizeof(sqrt_ratio_test_vector);
...
void test_h2c_3isogeny_mapping_vectors(void) {
    size_t i;
    size_t ietf_h2c_iso3map_num_test_vectors;

    ietf_h2c_iso3map_num_test_vectors = sizeof(iso3_test_vectors) / sizeof(iso3_test_vector);
...
struct expander_xmd_fixture {
    const ietf_rfc9380_expander_test_vector *tvs;
    size_t tvs_len;

    const unsigned char *dst;
    size_t dst_len;

    const unsigned char *dst_prime;
    size_t dst_prime_len;

    const unsigned char *msgs;
    const unsigned char *msg_primes;
    const unsigned char *uniform_bytes;
} ;

static void run_expand_message_xmd_vectors(const struct expander_xmd_fixture *fx) {
    size_t i;

    for (i = 0; i < fx->tvs_len; i++) {
        int result;
        ...
        msg_prime = (unsigned char *) checked_malloc(&default_error_callback,
                                                     ...
                                                     + tv->msg_len + 2
                                                     + 1 + dst_length + 1);
        compute_msg_prime(msg_prime, &fx->msgs[tv->msg_offset], tv->msg_len,
                          dst_prime, dst_length, len_in_bytes);
        result = memcmp(msg_prime, &fx->msg_primes[tv->msg_prime_offset], tv->msg_prime_len);
        /* check if they are equal */
        CHECK(result == 0);
        free(dst_);
        free(dst_prime);
        ...
        uniform_bytes = (unsigned char *) checked_malloc(&default_error_callback,
                                                         len_in_bytes);
        result = expand_message_xmd(uniform_bytes, &fx->msgs[tv->msg_offset], tv->msg_len,
                                    fx->dst, fx->dst_len,
                                    len_in_bytes);
        /* expand_message_xmd outputs 1 on success */
        CHECK(result == 1);
        result = memcmp(uniform_bytes, &fx->uniform_bytes[tv->uniform_bytes_offset], len_in_bytes);
        /* check if they are equal */
        CHECK(result == 0);
        free(uniform_bytes);
    }
...
void test_h2c_rfc9380_expand_message_xmd(void) {
    struct expander_xmd_fixture fx;

    fx.tvs = ietf_rfc9380_expander_test_vectors_dst;
    fx.tvs_len = sizeof(ietf_rfc9380_expander_test_vectors_dst) / sizeof(ietf_rfc9380_expander_test_vector);

    fx.dst = ietf_rfc9380_dst_exp_dst;
    fx.dst_len = sizeof(ietf_rfc9380_dst_exp_dst);

    fx.dst_prime = ietf_rfc9380_dst_exp_dst_prime;
    fx.dst_prime_len = sizeof(ietf_rfc9380_dst_exp_dst_prime);

    fx.msgs = ietf_rfc9380_dst_exp_msgs;
    fx.msg_primes = ietf_rfc9380_dst_exp_msg_primes;
    fx.uniform_bytes = ietf_rfc9380_dst_exp_uniform_bytes;

    run_expand_message_xmd_vectors(&fx);
}

/* Test expand_message_xmd against the RFC9380 test vector (long DST) */
void test_h2c_rfc9380_expand_message_xmd_long_dst(void) {
    struct expander_xmd_fixture fx;

    fx.tvs = ietf_rfc9380_expander_test_vectors_long_dst;
    fx.tvs_len = sizeof(ietf_rfc9380_expander_test_vectors_long_dst) / sizeof(ietf_rfc9380_expander_test_vector);

    fx.dst = ietf_rfc9380_long_dst_exp_dst;
    fx.dst_len = sizeof(ietf_rfc9380_long_dst_exp_dst);

    fx.dst_prime = ietf_rfc9380_long_dst_exp_dst_prime;
    fx.dst_prime_len = sizeof(ietf_rfc9380_long_dst_exp_dst_prime);

    fx.msgs = ietf_rfc9380_long_dst_exp_msgs;
    fx.msg_primes = ietf_rfc9380_long_dst_exp_msg_primes;
    fx.uniform_bytes = ietf_rfc9380_long_dst_exp_uniform_bytes;
...
void test_h2c_rfc9380_hash_to_curve_test_vectors(void) {
    int i;
    int ietf_rfc9380_ro_test_vectors_size;
    secp256k1_gej P;

    ietf_rfc9380_ro_test_vectors_size = sizeof(ietf_rfc9380_ro_test_vectors) / sizeof(ietf_rfc9380_test_vector);
    for (i = 0; i < ietf_rfc9380_ro_test_vectors_size; i++) {
        secp256k1_fe u[2];
        secp256k1_gej Q[2];
        ...
        CHECK(hash_to_field(u,
                      &ietf_rfc9380_ro_messages[tv->msg_offset], tv->msg_len,
                      ietf_rfc9380_ro_dst,
                      sizeof(ietf_rfc9380_ro_dst), 2) == 1);
...
void test_h2c_rfc9380_hash_to_curve_e2e_test_vectors(void) {
    ...
    ietf_rfc9380_ro_test_vectors_size = sizeof(ietf_rfc9380_ro_test_vectors) / sizeof(ietf_rfc9380_test_vector);
    for (i = 1; i < ietf_rfc9380_ro_test_vectors_size; i++) {
        const ietf_rfc9380_test_vector *tv;
        int result;
        tv = &ietf_rfc9380_ro_test_vectors[i];

        CHECK(hash_to_curve(&P,
                      &ietf_rfc9380_ro_messages[tv->msg_offset], tv->msg_len,
                      ietf_rfc9380_ro_dst,
                      sizeof(ietf_rfc9380_ro_dst)) == 1);
...
void test_h2c_rfc9380_encode_to_curve_test_vectors(void) {
    int i;
    int ietf_rfc9380_nu_test_vectors_size;

    ietf_rfc9380_nu_test_vectors_size = sizeof(ietf_rfc9380_nu_test_vectors) / sizeof(ietf_rfc9380_test_vector);
    for (i = 1; i < ietf_rfc9380_nu_test_vectors_size; i++) {
        secp256k1_fe u;
        secp256k1_gej Q;
        const ietf_rfc9380_test_vector *tv;
        unsigned char buffer[32];
        ...
        CHECK(hash_to_field(&u,
                      ...
                      ietf_rfc9380_nu_dst,
                      sizeof(ietf_rfc9380_nu_dst), 1) == 1);
...
void test_h2c_rfc9380_encode_to_curve_e2e_test_vectors(void) {
    ...
    for (i = 1; i < ietf_rfc9380_nu_test_vectors_size; i++) {
        ...
        CHECK(encode_to_curve(&P,
                        &ietf_rfc9380_nu_messages[tv->msg_offset], tv->msg_len,
                        ietf_rfc9380_nu_dst,
                        sizeof(ietf_rfc9380_nu_dst)) == 1);
