 ...
 */
static int hash_to_field(secp256k1_fe *field_elems,
                          const unsigned char *msg, uint32_t msg_length,
                          const unsigned char *dst, uint32_t dst_length,
                          uint32_t count) {
    ...
    if (uniform_bytes != NULL) {
        secp256k1_memclear(uniform_bytes, len_in_bytes);
        free(uniform_bytes);
    }
...
static int hash_to_scalar_field(unsigned char *out32,
                                 const unsigned char *msg, uint32_t msg_length,
                                 const unsigned char *dst, uint32_t dst_length) {
 ...
 *       dst_length: actual length of dst
 */
static int hash_to_curve(secp256k1_gej *P,
                          const unsigned char *msg, uint32_t msg_length,
                          const unsigned char *dst, uint32_t dst_length) {
    secp256k1_fe u[2];
    secp256k1_gej Q[2];
    if (hash_to_field(u, msg, msg_length, dst, dst_length, 2) == 0) {
        secp256k1_fe_clear(&u[0]);
        secp256k1_fe_clear(&u[1]);
        return 0;
    }
 ...
 *       dst_length: actual length of dst
 */
static int encode_to_curve(secp256k1_gej *P,
                          const unsigned char *msg, uint32_t msg_length,
                          const unsigned char *dst, uint32_t dst_length) {
    secp256k1_fe u;
    if (hash_to_field(&u, msg, msg_length, dst, dst_length, 1) == 0) {
