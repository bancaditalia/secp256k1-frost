...
static SECP256K1_WARN_UNUSED_RESULT int compute_group_commitment(
                                                      /* out */ secp256k1_gej *group_commitment,
...
static int encode_group_commitments(
    ...
    for (index = 0; index < num_signers; index++) {
        ...
        if (secp256k1_frost_pubkey_serialize(&(buffer[hiding_idx]), item.hiding) == 0) {
            return 0;
        }
        if (secp256k1_frost_pubkey_serialize(&(buffer[binding_idx]), item.binding) == 0) {
            return 0;
        }
    }
...
static SECP256K1_WARN_UNUSED_RESULT int compute_binding_factors(
    ...
    /* msg_hash = H4(msg) */
    compute_hash_h4(&rho_input[SERIALIZED_PUBKEY_X_PLUS_1_SIZE],
                    msg,
                    msg_length);
    ...
    for (index = 0; index < num_signers; index++) {
        ...
        serialize_scalar(&(rho_input[SERIALIZED_PUBKEY_X_PLUS_1_SIZE + SHA256_SIZE + SHA256_SIZE]),
...
static SECP256K1_WARN_UNUSED_RESULT int derive_interpolating_value(
                                                /* out */ secp256k1_scalar *lambda_i,
...
SECP256K1_API int secp256k1_frost_sign(
    ...
    if (compute_binding_factors(ctx,
                                ...
                                msg_length,
                                num_signers,
                                keypair->public_keys.group_public_key,
...
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_frost_verify(
    ...
    secp256k1_scalar challenge;
    secp256k1_gej lhs, rhs, term2, group_pubkey;
    secp256k1_frost_signature aggregated_signature;
    int is_valid;
    ...
    secp256k1_gej_add_var(&rhs, &(aggregated_signature.r), &term2, NULL);
