...
"""
Generate a C file with the FROST IETF test vectors (RFC9591).

Usage:
$ tools/frost_generate_ietf_rfc9591_test_vectors.py \
    src/modules/frost/vectors/ietf_frost_rfc9591_test_vectors.json \
    > src/modules/frost/vectors/ietf_frost_rfc9591_test_vectors.h
"""
...
print()
print("#ifndef VECTORS_IETF_FROST_RFC9591_TEST_VECTORS_H")
print("#define VECTORS_IETF_FROST_RFC9591_TEST_VECTORS_H")
print()
...
print(f"static const unsigned char ietf_frost_group_secret_key[]    = {{ {to_c_array(doc['group_input_parameters']['group_secret_key'])}}};")
print(f"static const unsigned char ietf_frost_group_public_key[]    = {{ {to_c_array(doc['group_input_parameters']['group_public_key'])}}};")
print(f"static const unsigned char ietf_frost_message[]    = {{ {to_c_array(doc['group_input_parameters']['message'])}}};")
print(f"static const size_t ietf_frost_message_length      = {len(doc['group_input_parameters']['message']) // 2};")
# TODO: improve representation of coefficients (threshold - 1)
print(f"static const unsigned char ietf_frost_share_polynomial_coefficients_0[]  = {{ {to_c_array(doc['group_input_parameters']['share_polynomial_coefficients'][0])}}};")
print()

print("/* Section: signer_input_parameters */")

print(f"#define IETF_FROST_PARTICIPANT_SHARE_SIZE {len(doc['signer_input_parameters']['participant_share'][0]) // 2}")
...
len_hnr = len(signer_outputs["participant_1"]["hiding_nonce_randomness"]) // 2
len_hn = len(signer_outputs["participant_1"]["hiding_nonce"]) // 2
len_bnr = len(signer_outputs["participant_1"]["binding_nonce_randomness"]) // 2
len_bn = len(signer_outputs["participant_1"]["binding_nonce"]) // 2
len_hnc = len(signer_outputs["participant_1"]["hiding_nonce_commitment"]) // 2
len_bnc = len(signer_outputs["participant_1"]["binding_nonce_commitment"]) // 2
len_bfi = len(signer_outputs["participant_1"]["binding_factor_input"]) // 2
len_bf = len(signer_outputs["participant_1"]["binding_factor"]) // 2

print(f"#define IETF_FROST_HIDING_NONCE_RANDOMNESS_SIZE {len_hnr}")
print(f"#define IETF_FROST_HIDING_NONCE_SIZE {len_hn}")
print(f"#define IETF_FROST_BINDING_NONCE_RANDOMNESS_SIZE {len_bnr}")
print(f"#define IETF_FROST_BINDING_NONCE_SIZE {len_bn}")
print(f"#define IETF_FROST_HIDING_NONCE_COMMITMENT_SIZE {len_hnc}")
print(f"#define IETF_FROST_BINDING_NONCE_COMMITMENT_SIZE {len_bnc}")
print(f"#define IETF_FROST_BINDING_FACTOR_INPUT_SIZE {len_bfi}")
print(f"#define IETF_FROST_BINDING_FACTOR_SIZE {len_bf}")
...
len_sig_share = len(signer_outputs["participant_1"]["sig_share"]) // 2
print(f"#define IETF_FROST_SIG_SHARE_SIZE {len_sig_share}")

len_sig = len(sig) // 2
print(f"#define IETF_FROST_SIG_SIZE {len_sig}")
...
print()
print("#endif /* VECTORS_IETF_FROST_RFC9591_TEST_VECTORS_H */")
